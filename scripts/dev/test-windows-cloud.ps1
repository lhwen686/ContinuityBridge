[CmdletBinding()]
param([switch]$DesktopFixtures, [switch]$SkipRestore)
...
try {
    ...
    & $sdk test tests/ContinuityBridge.Core.Tests/ContinuityBridge.Core.Tests.csproj -c Release --no-restore --logger 'trx;LogFileName=core.trx' --results-directory artifacts/p5/final
    if ($LASTEXITCODE -ne 0) { throw 'Core tests failed.' }
    & $sdk test tests/ContinuityBridge.Windows.Tests/ContinuityBridge.Windows.Tests.csproj -c Release --no-restore --filter 'TestCategory=P5' --logger 'trx;LogFileName=windows-http-image.trx' --results-directory artifacts/p5/final
    ...
    if ($DesktopFixtures) {
        ...
        & $sdk test tests/ContinuityBridge.Windows.Tests/ContinuityBridge.Windows.Tests.csproj -c Release --no-restore --no-build --filter 'TestCategory=P5Desktop' --logger 'trx;LogFileName=desktop.trx' --results-directory artifacts/p5/final
