...
最小修补方案 `M2-LANGUAGE-REPAIR-20260919-R1` 已准备，等待产品修改及独立回归批准：仅把发送模板两个严格相等条件改为同一输入的中英文任一匹配（`图像`/`Image`、`文本`/`Text`），保留未知类型拒绝及所有协议、原对象、完整性检查。原生编辑可能立即经既有 iCloud 同步到实际产品副本，因此在具体批准前不编辑正式模板。当前报告提交不包含语言修补，不能冻结候选。取回模板未发现这些类型条件，不拟修改。
...
- Mac 的批准范围内构建、签名、安装、用户信任和真机工具恢复已完成；英文离线探针已完成并发现兼容缺口；产品语言修补批准及既有菜单入口引用仍待完成。
- 产品备份、两处具体条件差异、原生导出与手机核对步骤、限定恢复办法已在本机方案中给出；获批后修补和离线回归，不用正式 QA 验证准备阶段的问题。
